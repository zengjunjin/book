<template>
  <div class="recommend-view">
    ...
    <div v-if="userStore.isLoggedIn" class="status-card">
      ...
      <div class="status-item">
        <el-icon :size="24" :color="'#71717a'"><InfoFilled /></el-icon>
        <div>
...
const fetchUserRatingCount = async () => {
  ...
  try {
    const res = await ratingAPI.getUserRatings(userStore.user.id, 1, 1)
...
const fetchRecommendations = async (showToast = false) => {
  ...
  try {
    const [cfRes, svdRes] = await Promise.all([
      recommendAPI.getCFRecommendations(userStore.user.id, 20, true),
      recommendAPI.getSVDRecommendations(userStore.user.id, 20, true)
    ])
